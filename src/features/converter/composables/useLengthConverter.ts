...
export interface UseLengthConverterReturn {
  // State (readonly)
...
export function useLengthConverter(): UseLengthConverterReturn {
  // Load options
  ...
  const availableUnits = computed<ConversionUnit[]>(() =>
    ConverterRegistry.getInstance().get('length')?.units || []
  )
