...
export interface UseWeightConverterReturn {
  // State (readonly)
...
export function useWeightConverter(): UseWeightConverterReturn {
  // Load options
  const converterOptions = useConverterOptions()

  // State
  const inputValue = ref<string>('0')
  const fromUnit = ref<string>('kilogram')
  const toUnit = ref<string>('pound')
  ...
  const availableUnits = computed<ConversionUnit[]>(() =>
    ConverterRegistry.getInstance().get('weight')?.units || []
  )
