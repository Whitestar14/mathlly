  ...
  <BaseEditor
    ...
    <template #toolbar>
      ...
      <div class="flex items-center gap-1">
        <BaseButton
          v-if="hasPreview" v-tippy="showPreview ? 'Show Text' : 'Show Preview'" variant="ghost" size="icon"
        ...
        <BaseButton
          v-tippy="'Swap'" variant="ghost" size="icon" class="size-8" :disabled="!modelValue || !!error"
          @click="$emit('swap')">
          <ArrowDownUp class="size-4 text-muted-foreground" />
        </BaseButton>
        <BaseButton
          v-tippy="'Download'" variant="ghost" size="icon" class="size-8" :disabled="!modelValue || !!error"
          @click="$emit('download')">
          <Download class="size-4 text-muted-foreground" />
        </BaseButton>
        <BaseButton
          v-tippy="'Copy'" variant="ghost" size="icon" class="size-8" :disabled="!modelValue || !!error"
          @click="$emit('copy')">
    ...
    <template v-if="(showPreview && hasPreview && !error) || (showBinaryWarning && !error)" #default>
      ...
      <div v-if="showPreview && hasPreview && !error" class="flex-1 flex flex-col items-center justify-center p-4 bg-muted/5 w-full h-full">
        <div class="relative max-w-full max-h-full flex flex-col items-center justify-center gap-4">
          <img
            v-if="previewUrl && previewInfo?.mime.startsWith('image/')" :src="previewUrl"
            class="max-h-[300px] object-contain rounded-md border border-border shadow-sm bg-[url('/img/transparent-grid.png')]" />
          <iframe
            v-else-if="previewUrl && previewInfo?.mime === 'application/pdf'" :src="previewUrl"
      ...
      <div v-if="showBinaryWarning && !error" class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/95 p-6 text-center space-y-3 animate-in fade-in">
        ...
        <BaseButton variant="secondary" size="sm" class="pointer-events-auto" @click="forceShowText = true">
          Show Anyway
        </BaseButton>
      </div>
...
import { Copy, Download, ArrowDownUp, Eye as EyeIcon, FileText as FileTextIcon, File as FileIcon, AlertTriangle } from 'lucide-vue-next'
...
defineEmits(['copy', 'download', 'swap'])
