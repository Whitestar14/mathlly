        ...
        <div class="container mx-auto p-2 md:p-3">
            <div class="max-w-6xl mx-auto space-y-3">
                <div class="rounded-lg border border-border dark:border-border overflow-hidden">
                    ...
                    <BaseTabs ref="tabsRef" v-model:model-value="tool.currentTab.value" :tabs="tabs"
                        @tab-change="handleTabChange">
                        <template #actions>
                            <Base64Actions :options="tool.options.value"
                                ...
                                :clear-all="tool.handleClear" :trigger-file-picker="triggerUiFilePicker" />
                        </template>
                    </BaseTabs>

                    <div class="p-3 md:p-6 bg-card">
                        <!-- Hidden File Input -->
                        <input ref="fileInputRef" type="file" class="hidden" @change="tool.onFileUpload" />

                        <FileProcessingOverlay :open="tool.isFileProcessing.value" />

                        <div class="relative">
                            <!-- Drag Overlay -->
                            <div v-if="tool.fileUI.isDragActive.value"
                                ...
                                <Transition name="fade-scale" appear>
                                    <FileUpload :handle-binary-files="true" :current-tab="tool.currentTab.value"
                                        class="w-full max-w-4/5 mx-auto" @file-upload="tool.onFileUpload"
                                        @drop="handleDropWrapper" />
                                </Transition>
                            </div>

                            <!-- Main Grid -->
                            <div :class="{ invisible: tool.fileUI.isDragActive.value }"
                                class="grid gap-2 md:gap-3 lg:grid-cols-2">

                                <!-- Input Panel -->
                                <BaseCard class="border-none">
                                    <TextPanel ref="inputArea" v-model="tool.input.value"
                                        ...
                                        :validation-error="tool.ops.validationError.value" :show-paste-button="true"
                                        @input="tool.handleInput" @drop="handleDropWrapper" @paste="handleUiPaste">
                                        <template #actions>
                                            <div class="flex items-center gap-2">
                                                <BaseButton v-if="!tool.options.value.autoProcess" variant="outline"
                                                    :disabled="tool.ops.isProcessing.value" @click="handleProcess">
                                                    <Loader2 v-if="tool.ops.isProcessing.value"
                                                        class="size-4 animate-spin" />
                                                    {{ tool.currentTab.value === 'encode' ? 'Encode' : 'Decode' }}
                                                </BaseButton>

                                                <BaseButton v-if="tool.input.value" variant="ghost"
                                                    class="size-8 hover:bg-destructive/10 hover:text-destructive"
                                                    size="icon" @click="() => {
                                                        tool.input.value = ''
                                                        tool.selectedFileName.value = ''
                                                    }">
                                                    <X class="size-4" />
                                                </BaseButton>
                                            </div>
                                ...
                                <!-- Output Panel -->
                                <BaseCard class="border-none">
                                    <TextPanel :model-value="tool.ops.output.value"
                                        :label="`Output (${outputFormatLabel})`"
                                        :placeholder="'Result will appear here...'" :stats="tool.ops.outputStats.value"
                                        :show-stats="tool.options.value.showCharacterCount"
                                        :validation-error="tool.outputValidationError.value" :read-only="true">
                                        <template #actions>
                                            <div class="flex items-center gap-2">
                                                <BaseButton v-if="tool.activePreviewUrl.value || previewInfo"
                                                ...
                                                <div v-if="tool.activePreviewUrl.value || previewInfo"
                                                    class="h-4 w-px bg-border mx-1"></div>

                                                <BaseButton v-tippy="{ content: 'Swap' }" variant="ghost" size="icon"
                                                    class="size-8" :disabled="!tool.ops.output.value"
                                                    @click="tool.handleSwap">
                                                    <ArrowDownUp class="size-4" />
                                                </BaseButton>

                                                <BaseButton v-tippy="{ content: 'Download' }" variant="ghost"
                                                ...
                                                <BaseButton v-tippy="{ content: 'Copy' }" variant="ghost" size="icon"
                                                    class="size-8" :disabled="!tool.ops.output.value"
                                        ...
                                        <template #content>
                                            <div class="relative flex-1">
                                                <!-- File Preview -->
                                                <template
                                                    v-if="showPreview && (tool.activePreviewUrl.value || previewInfo)">
                                                    <div
                                                        class="w-full h-full min-h-[192px] rounded-md border border-border bg-background p-4 flex flex-col items-center justify-center gap-4">
                                                        <img v-if="tool.activePreviewUrl.value && previewInfo?.mime.startsWith('image/')"
                                                        ...
                                                        <div class="text-xs text-muted-foreground text-center">
                                                            <p class="font-medium text-foreground">{{ previewInfo?.mime
                                                                }}</p>
                                                ...
                                                <textarea
                                                    ...
                                                    :value="tool.ops.output.value" readonly
