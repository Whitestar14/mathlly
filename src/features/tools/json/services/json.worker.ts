...
self.onmessage = (e: MessageEvent<WorkerMessage>) => {
  ...
  try {
    if (type === 'PROCESS') {
      ...
      try { ts = generateTypeScript(parsed) } catch { ts = '// Error generating TypeScript' }
      try { xml = toXml(parsed, indent) } catch { xml = '<!-- Error generating XML -->' }
      try { csv = toCsv(parsed) } catch { csv = 'Error generating CSV' }
