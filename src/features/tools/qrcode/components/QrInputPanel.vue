    ...
    <div class="flex-1 overflow-y-auto min-h-0 pr-1 space-y-4">
      ...
      <div class="space-y-4 p-1">
        ...
        <BaseEditor
          ...
          :stats="''"
          @update:model-value="val => updateContentField('text', val)" />

        <!-- URL / PDF MODE -->
        <div v-if="content.type === 'url' || content.type === 'pdf'" class="space-y-2">
          <BaseLabel>{{ content.type === 'pdf' ? 'PDF File URL' : 'Website URL' }}</BaseLabel>
          <BaseInput :model-value="content.url" placeholder="https://example.com/file.pdf" :icon="LinkIcon" @update:model-value="val => updateContentField('url', val)" />
        ...
        <div v-if="content.type === 'app'" class="space-y-3 border border-border bg-card/50 rounded-lg p-4">
          ...
          <div class="space-y-1.5">
            ...
            <BaseInput :model-value="content.app.appId" :placeholder="content.app.platform === 'ios' ? '123456789' : 'com.example.app'" @update:model-value="val => updateAppField('appId', val)" />
          </div>
        </div>
        ...
        <div v-if="content.type === 'wifi'" class="space-y-3 border border-border bg-card/50 rounded-lg p-4">
          <div class="space-y-1.5">
            ...
            <BaseInput :model-value="content.wifi.ssid" placeholder="MyWiFi" :icon="Wifi" @update:model-value="val => updateWifiField('ssid', val)" />
          </div>
          <div class="space-y-1.5">
            <BaseLabel>Password</BaseLabel>
            <BaseInput :model-value="content.wifi.password" type="password" placeholder="Password" :icon="Lock" @update:model-value="val => updateWifiField('password', val)" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <BaseLabel>Encryption</BaseLabel>
              <SelectBar
                :model-value="content.wifi.encryption"
                :options="[{value: 'WPA/WPA2', label: 'WPA/WPA2'}, {value: 'WEP', label: 'WEP'}, {value: 'None', label: 'None'}]"
                @update:model-value="val => updateWifiField('encryption', val)" />
            </div>
            <div class="flex items-center gap-2 pt-6">
              <input id="hidden-wifi" type="checkbox" :checked="content.wifi.hidden" class="rounded border-border text-primary focus:ring-primary/20" @change="e => updateWifiField('hidden', (e.target as HTMLInputElement).checked)" />
        ...
        <div v-if="content.type === 'email'" class="space-y-3 border border-border bg-card/50 rounded-lg p-4">
          <div class="space-y-1.5">
            ...
            <BaseInput :model-value="content.email.to" placeholder="user@example.com" :icon="Mail" @update:model-value="val => updateEmailField('to', val)" />
          </div>
          <div class="space-y-1.5">
            <BaseLabel>Subject</BaseLabel>
            <BaseInput :model-value="content.email.subject" placeholder="Hello..." @update:model-value="val => updateEmailField('subject', val)" />
          </div>
          <div class="space-y-1.5">
            ...
            <textarea
              ...
              @input="e => updateEmailField('body', (e.target as HTMLTextAreaElement).value)"></textarea>
          </div>
        </div>
        ...
        <div v-if="content.type === 'twitter'" class="space-y-3 border border-border bg-card/50 rounded-lg p-4">
          <div class="space-y-1.5">
            ...
            <textarea
              ...
              maxlength="280"
              @input="e => updateTwitterField('text', (e.target as HTMLTextAreaElement).value)"></textarea>
            <p class="text-xs text-right text-muted-foreground">{{ content.twitter.text.length }}/280</p>
          </div>
          <div class="space-y-1.5">
            <BaseLabel>URL (Optional)</BaseLabel>
            <BaseInput :model-value="content.twitter.url" placeholder="https://..." @update:model-value="val => updateTwitterField('url', val)" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <BaseLabel>Hashtags</BaseLabel>
              <BaseInput :model-value="content.twitter.hashtags" placeholder="tech,code" @update:model-value="val => updateTwitterField('hashtags', val)" />
            </div>
            <div class="space-y-1.5">
              <BaseLabel>Via (@username)</BaseLabel>
              <BaseInput :model-value="content.twitter.via" placeholder="prism" @update:model-value="val => updateTwitterField('via', val)" />
            </div>
          </div>
        </div>
        ...
        <div v-if="content.type === 'crypto'" class="space-y-3 border border-border bg-card/50 rounded-lg p-4">
          <div class="space-y-1.5">
            ...
            <SelectBar
              :model-value="content.crypto.currency"
              :options="[
                { value: 'bitcoin', label: 'Bitcoin (BTC)' },
                { value: 'ethereum', label: 'Ethereum (ETH)' },
                { value: 'solana', label: 'Solana (SOL)' }
              ]"
              @update:model-value="val => updateCryptoField('currency', val)" />
          </div>
          <div class="space-y-1.5">
            <BaseLabel>Wallet Address</BaseLabel>
            <BaseInput :model-value="content.crypto.address" placeholder="Address" :icon="Wallet" @update:model-value="val => updateCryptoField('address', val)" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1.5">
              <BaseLabel>Amount</BaseLabel>
              <BaseInput :model-value="content.crypto.amount" placeholder="0.00" type="number" @update:model-value="val => updateCryptoField('amount', val)" />
            </div>
            <div class="space-y-1.5">
              <BaseLabel>Label (Optional)</BaseLabel>
              <BaseInput :model-value="content.crypto.label" placeholder="Donation" @update:model-value="val => updateCryptoField('label', val)" />
            </div>
          </div>
        </div>
          ...
          <div class="space-y-4 pt-2 px-1">
            ...
            <div class="grid grid-cols-2 gap-4 items-start">
              ...
              <div v-if="options.frame === 'bottom-text'" class="space-y-1.5 animate-in fade-in slide-in-from-top-1">
                ...
                <BaseInput :model-value="options.frameText" placeholder="SCAN ME" @update:model-value="updateOption('frameText', $event)" />
              </div>
            </div>
...
const updateContentField = (key: keyof QrContentState, value: any) => {
  emit('update:content', { ...props.content, [key]: value })
}

const updateWifiField = (key: string, value: any) => {
  emit('update:content', { ...props.content, wifi: { ...props.content.wifi, [key]: value } })
}

const updateEmailField = (key: string, value: any) => {
  emit('update:content', { ...props.content, email: { ...props.content.email, [key]: value } })
}

const updateTwitterField = (key: string, value: any) => {
  emit('update:content', { ...props.content, twitter: { ...props.content.twitter, [key]: value } })
}

const updateCryptoField = (key: string, value: any) => {
  emit('update:content', { ...props.content, crypto: { ...props.content.crypto, [key]: value } })
}

const updateAppField = (key: string, value: any) => {
  emit('update:content', { ...props.content, app: { ...props.content.app, [key]: value } })
}
