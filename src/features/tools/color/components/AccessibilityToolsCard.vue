<template>
    <BaseCard title="Accessibility Tools">
      ...
      <div class="mt-4">
        ...
        <div v-if="activeTab === 'contrast'" class="space-y-4">
          ...
          <div class="flex items-center gap-2">
            <BaseBadge :class="`${contrastLevel.color} text-white`" :text="contrastLevel.level" />
            <span class="text-xs text-muted-foreground">
              WCAG {{ contrastLevel.level === 'Fail' ? 'Non-compliant' : 'Compliant' }}
            </span>
          </div>
          <div
  ...
  import { convertColor, simulateColorBlindness, getContrastRatio } from '@color/composables/useColor'
