...
const getUpdateDescription = (): string => {
  ...
  if (
    ...
    if (
      currentVersion.value.includes('beta') &&
      !latestVersion.value.includes('beta')
    ) {
      return 'Stable release ready - enhanced stability and performance';
    }

    if (updateFeatures.value.length > 0) {
      const featureText = isMobile.value
        ? 'updates'
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mb-3 overflow-hidden">
                ...
                >
                  <span
                    class="w-1.5 h-1.5 bg-primary rounded-full mr-1.5 animate-pulse"
                  />
          ...
          >
            ...
            <div class="p-4">
              <div class="max-h-40 overflow-y-auto">
                <h4
                  class="text-xs font-medium text-card-foreground mb-3 flex items-center"
                >
                  <div class="w-1 h-1 bg-primary rounded-full mr-2" />
                  {{
                    isServiceWorkerUpdate
                      ? 'Service Worker Updates'
                      : `What's new in ${formatVersion(displayLatestVersion)}`
                  }}
                </h4>
                ...
                <div class="space-y-2">
                  ...
                  >
                    <CheckIcon
                      class="h-3 w-3 text-primary mt-0.5 flex-shrink-0"
                    />
                    <span
